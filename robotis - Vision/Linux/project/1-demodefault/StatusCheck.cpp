...
int modestartlama, powerlama;
int modepilihanlama = READY;

void StatusCheck::Check(CM730 &cm730, int modepilihan, int modestart, int power, bool* abisjatoh)
{
    struct timeval _tv;
    ...
    //        Walking::GetInstance()->Stop();
            cm730.WriteByte(254, MX28::P_TORQUE_ENABLE, 1, NULL);
            usleep(1000000);
    ...
    if(MotionStatus::FALLEN != STANDUP && (m_cur_mode == SOCCER || m_cur_mode == VISION || m_cur_mode == MOTION) && m_is_started != 0)
    {
        Walking::GetInstance()->Stop();
    ...
    if((m_cur_mode == SOCCER || m_cur_mode == VISION || m_cur_mode == MOTION) && m_is_started == 2)
    {
    ...
    // if(modepilihanlama != modepilihan)
    // {
    //     if(m_is_started == 1)
    //     {
    //         m_is_started = 0;
    ...
    if(m_old_btn & BTN_MODE || modepilihanlama != modepilihan)
    {
        fprintf(stderr, "Mode button pressed.. \n");

        if(m_is_started != 0 || modepilihan == 0)
        {
            m_is_started    = 0;
            m_cur_mode      = READY;
        ...
        else if(modepilihanlama != modepilihan)
        {
            switch(modepilihan)
            {
                case 0: m_cur_mode = READY; break;
                case 1: m_cur_mode = READY; break;
                case 2: m_cur_mode = SOCCER; break;
                case 3: m_cur_mode = MOTION; break;
                case 4: m_cur_mode = VISION; break;
                case 5: m_cur_mode = MAX_MODE; break;
            }
    ...
    modepilihanlama = modepilihan;
    // printf("\nMode pilihan       : %d", modepilihan);
    // printf("\nMode pilihan lama  : %d", modepilihanlama);
    // printf("\nPower              : %d", power);
    // printf("\nPower lama         : %d", powerlama);
    if(modestart != modestartlama && modestart == 1)
    {
        m_old_btn = 2;
        m_is_started = 0;
    }
