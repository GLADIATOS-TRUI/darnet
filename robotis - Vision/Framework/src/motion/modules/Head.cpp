...
Head::Head()
{
	...
	m_LeftLimit = 70;
	m_RightLimit = -70;
	m_TopLimit = Kinematics::EYE_TILT_OFFSET_ANGLE - 40;
	m_BottomLimit = Kinematics::EYE_TILT_OFFSET_ANGLE - 85;

	m_Pan_Home = 0.0;
	m_Tilt_Home = Kinematics::EYE_TILT_OFFSET_ANGLE -30; //-65;//- 30.0;
...
{
	m_PanAngle = 0;//MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_PAN);
	m_TiltAngle = -0;//MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_TILT);
	CheckLimit();
...
void Head::MoveTracking()
{
	...
	CheckLimit();
	SerialHandler::GetInstance()->SendHead(m_PanAngle, m_TiltAngle);
}
