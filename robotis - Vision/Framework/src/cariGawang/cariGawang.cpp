...
void cariGawangMK2(int posisiGawang[2][2], Mat imgGawang, Mat imgIjo, double soedoet, double &soedoetZ){
	...
	soedoet+=50; //di darwinOP, liat bawah itu -25 kok bukan 25???
	//printf("%.2f\n",soedoet);
	printf("masoook sini yok %.2f\n",soedoet);
	if(soedoet>=46.7 && soedoet <=133.3){
		nyari = (soedoet-46.7)/90;
		soedoetZ=nyari;
		cout<<"nah gitu dong "<<nyari<<endl;
		yStart = ((1-nyari) * imgSize.height) + 32;
		y =yStart; 
		if(yStart<=0)return;
		if(y<=0)y=1;
		if(y>=imgSize.height)y=imgSize.height-1;
		for(i = 0; i<cari; i++){
			...
			for(x=0;x<=imgSize.width;x++){
				//printf("%d-",img.at<uchar>(x,y));
				if(cariPutih==true){
					if(x==imgSize.width-5)break;
					printf("%d ",imgGawang.at<uchar>(y,x));
					if (imgGawang.at<uchar>(y,x)==255){
						cout<<x<<" "<<endl;
						if(dimanaGawang[i][0]<=0){
							dimanaGawang[i][0] = x;
							cout<<"hah "<<x<<endl;
							cariPutih=false;
							continue;
						}else if(dimanaGawang[i][1]<=0){
							dimanaGawang[i][1] = x;
							cariPutih=false;
							continue;
						}else continue;
					...
					if (imgGawang.at<uchar>(y,x)==0){
						cout<<x<<" "<<endl;
						if(dimanaGawang[i][1]<=0){
							dimanaGawang[i][0] = (dimanaGawang[i][0]+x)/2;
							cariPutih=true;
							continue;
						}else{
							dimanaGawang[i][1] = (dimanaGawang[i][1]+x)/2;
							cariPutih=true;
							break;
						}
						cariPutih=true;
		...
		for(int i = 0; i<cari; i++){
			cout<<ngitung[0]<<endl;
			ngitung[0] += dimanaGawang[i][0];
			ngitung[1] += dimanaGawang[i][1];
		}
		if(ngitung[0]/cari>0)nemuBerapa++;
		if(ngitung[1]/cari>0)nemuBerapa++;
		
		cout<<"nemu berapa garis?"<<nemuBerapa<<endl;
		//if(ngitung[0]<0||ngitung[1]<0)return;
		if(nemuBerapa==0)return;
		posisiGawang[0][0] = (ngitung[0]/cari);
		cout<<"we ada gawang "<<posisiGawang[0][0]<<endl;
		if(nemuBerapa==2){
			posisiGawang[1][0] = (ngitung[1]/cari);
			cout<<" sama "<<posisiGawang[1][0]<<endl;
			tengah = (posisiGawang[0][0]+posisiGawang[1][0])/2;
		}
		for(y=yStart;y>0;y--){
			posisiGawang[0][1]=0;
			if (imgGawang.at<uchar>(y,posisiGawang[0][0])==0){
				posisiGawang[0][1]=y;
				break;
			}
		}if (nemuBerapa==1)return;
		for(y=yStart;y>0;y--){
			posisiGawang[1][1]=0;
			if (imgGawang.at<uchar>(y,posisiGawang[1][1])==0){
				posisiGawang[1][1]=y;
				break;
			}
		}/*
