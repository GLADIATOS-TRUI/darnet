...
void JointData::SetValue(int id, int value)
{
    ...
    if(id>=1&&id<=18)
    {
        if(value < MX28::MXMIN_VALUE)
            value = MX28::MXMIN_VALUE;

        else if(value >= MX28::MXMAX_VALUE)
            value = MX28::MXMAX_VALUE;

        m_Value[id] = value;
        m_Angle[id] = MX28::MXValue2Angle(value);
    }
...
void JointData::SetAngle(int id, double angle)
{
    if(id>=1&&id<=18)
    {
        if(angle < MX28::MXMIN_ANGLE)
            angle = MX28::MXMIN_ANGLE;

        else if(angle > MX28::MXMAX_ANGLE)
            angle = MX28::MXMAX_ANGLE;

        m_Angle[id] = angle;
        m_Value[id] = MX28::MXAngle2Value(angle);
